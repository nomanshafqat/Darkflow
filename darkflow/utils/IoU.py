...
def find_accuracy(detected,groundtruth,path):
    dumps=pascal_voc_clean_xml(path,['car'],parseOne=path)
    ...
    for i in range(0, len(groundtruth)):
        ...
        for j in range(0, len(detected)):
            IoU=bb_intersection_over_union(groundtruth[i][1:],detected[j])
            #print IoU
            if IoU>max:
                max=IoU
                match=j
        if max<0.4:
            undetected.append(groundtruth[i][1:])
        elif max>=0.4:
            True_Positive.append((groundtruth[i][1:],detected[match]))
