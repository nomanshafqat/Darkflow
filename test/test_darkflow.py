...
if buildPath is None:
    ...
    print(
        "If you want to test this locally, set TRAVIS_BUILD_DIR to the base directory of the cloned darkflow repository.")
    exit()

testImg = {"path": os.path.join(buildPath, "sample_img", "sample_person.jpg"), "width": 640, "height": 424,
           "expected-objects": {"yolo-small": [{"label": "dog", "confidence": 0.46, "topleft": {"x": 84, "y": 249},
                                                "bottomright": {"x": 208, "y": 367}},
                                               {"label": "person", "confidence": 0.60, "topleft": {"x": 159, "y": 102},
                                                "bottomright": {"x": 304, "y": 365}}],
                                "yolo": [{"label": "person", "confidence": 0.82, "topleft": {"x": 189, "y": 96},
                                          "bottomright": {"x": 271, "y": 380}},
                                         {"label": "dog", "confidence": 0.79, "topleft": {"x": 69, "y": 258},
                                          "bottomright": {"x": 209, "y": 354}},
                                         {"label": "horse", "confidence": 0.89, "topleft": {"x": 397, "y": 127},
                                          "bottomright": {"x": 605, "y": 352}}]}}

trainImgBikePerson = {"path": os.path.join(buildPath, "test", "training", "images", "1.jpg"), "width": 500,
                      "height": 375,
                      "expected-objects": {"tiny-yolo-voc": [
                          {"label": "bicycle", "confidence": 0.35, "topleft": {"x": 121, "y": 126},
                           "bottomright": {"x": 233, "y": 244}},
                          {"label": "person", "confidence": 0.60, "topleft": {"x": 132, "y": 35},
                           "bottomright": {"x": 232, "y": 165}}]}}

trainImgHorsePerson = {"path": os.path.join(buildPath, "test", "training", "images", "2.jpg"), "width": 500,
                       "height": 332,
                       "expected-objects": {"tiny-yolo-voc": [
                           {"label": "horse", "confidence": 0.99, "topleft": {"x": 156, "y": 108},
                            "bottomright": {"x": 410, "y": 281}},
                           {"label": "person", "confidence": 0.89, "topleft": {"x": 258, "y": 52},
                            "bottomright": {"x": 300, "y": 218}}]}}
...
def download_file(url, savePath):
    ...
    if not os.path.isfile(savePath):
        os.makedirs(os.path.dirname(savePath),
                    exist_ok=True)  # Make directories nessecary for file incase they don't exist
...
yolo_small_CfgPath = os.path.join(buildPath, "cfg", "v1",
                                  "{0}.cfg".format(os.path.splitext(os.path.basename(yolo_small_WeightPath))[0]))
...
tiny_yolo_voc_CfgPath = os.path.join(buildPath, "cfg",
                                     "{0}.cfg".format(os.path.splitext(os.path.basename(tiny_yolo_voc_WeightPath))[0]))
...
download_file(yolo_small_Download,
              yolo_small_WeightPath)  # Check if we need to download (and if so download) the yolo-small weights (YOLOv1)
download_file(yolo_Download,
              yolo_WeightPath)  # Check if we need to download (and if so download) the yolo weights (YOLOv2)
download_file(tiny_yolo_voc_Download,
              tiny_yolo_voc_WeightPath)  # Check if we need to download (and if so download) the tiny-yolo-voc weights (YOLOv2)
...
filelist = [f for f in os.listdir(os.path.dirname(testImg["path"])) if
            os.path.isfile(os.path.join(os.path.dirname(testImg["path"]), f)) and f != os.path.basename(
                testImg["path"])]
for f in filelist:
...
def test_CLI_IMG_YOLOv2():
    ...
    testString = "flow --imgdir {0} --model {1} --load {2} --config {3} --threshold 0.4".format(
        os.path.dirname(testImg["path"]), yolo_CfgPath, yolo_WeightPath, generalConfigPath)
...
def test_CLI_JSON_YOLOv2():
    ...
    testString = "flow --imgdir {0} --model {1} --load {2} --config {3} --threshold 0.4 --json".format(
        os.path.dirname(testImg["path"]), yolo_CfgPath, yolo_WeightPath, generalConfigPath)
    executeCLI(testString)

    outputJSONPath = os.path.join(os.path.dirname(testImg["path"]), "out",
                                  os.path.splitext(os.path.basename(testImg["path"]))[0] + ".json")
    ...
    assert compareObjectData(testImg["expected-objects"]["yolo"], loadedPredictions, testImg["width"],
                             testImg["height"], threshCompareThreshold,
                             posCompareThreshold), "Generated object predictions from JSON were not within margin of error compared to expected values."
...
def test_CLI_JSON_YOLOv1():
    ...
    testString = "flow --imgdir {0} --model {1} --load {2} --config {3} --threshold 0.4 --json".format(
        os.path.dirname(testImg["path"]), yolo_small_CfgPath, yolo_small_WeightPath, generalConfigPath)
    executeCLI(testString)
    ...
    assert compareObjectData(testImg["expected-objects"]["yolo-small"], loadedPredictions, testImg["width"],
                             testImg["height"], threshCompareThreshold,
                             posCompareThreshold), "Generated object predictions from JSON were not within margin of error compared to expected values."
    os.remove(outputJSONPath)  # Remove the JSON file so that it does not affect subsequent tests
...
def test_CLI_SAVEPB_YOLOv2():
    ...
    testString = "flow --model {0} --load {1} --config {2} --threshold 0.4 --savepb".format(yolo_CfgPath,
                                                                                            yolo_WeightPath,
                                                                                            generalConfigPath)
...
def test_RETURNPREDICT_PBLOAD_YOLOv2():
    ...
    assert compareObjectData(testImg["expected-objects"]["yolo"], loadedPredictions, testImg["width"],
                             testImg["height"], threshCompareThreshold,
                             posCompareThreshold), "Generated object predictions from return_predict() were not within margin of error compared to expected values."
...
def test_TRAIN_FROM_WEIGHTS_CLI__LOAD_CHECKPOINT_RETURNPREDICT_YOLOv2():
    ...
    testString = "flow --model {0} --load {1} --train --dataset {2} --annotation {3} --epoch 20".format(
        tiny_yolo_voc_CfgPath, tiny_yolo_voc_WeightPath, os.path.join(buildPath, "test", "training", "images"),
        os.path.join(buildPath, "test", "training", "annotations"))
    with pytest.raises(SystemExit):
        executeCLI(testString)
    ...
    loadedPredictions = tfnet.return_predict(imgcv)
    assert compareObjectData(trainImgBikePerson["expected-objects"]["tiny-yolo-voc"], loadedPredictions,
                             trainImgBikePerson["width"], trainImgBikePerson["height"], 0.7,
                             0.25), "Generated object predictions from training (for image with person on the bike) were not anywhere close to what they are expected to be.\nTraining may not have completed successfully."
    differentThanExpectedBike = compareObjectData(trainImgBikePerson["expected-objects"]["tiny-yolo-voc"],
                                                  loadedPredictions, trainImgBikePerson["width"],
                                                  trainImgBikePerson["height"], 0.01, 0.001)
    ...
    loadedPredictions = tfnet.return_predict(imgcv)
    assert compareObjectData(trainImgHorsePerson["expected-objects"]["tiny-yolo-voc"], loadedPredictions,
                             trainImgHorsePerson["width"], trainImgHorsePerson["height"], 0.7,
                             0.25), "Generated object predictions from training (for image with person on the horse) were not anywhere close to what they are expected to be.\nTraining may not have completed successfully."
    differentThanExpectedHorse = compareObjectData(trainImgHorsePerson["expected-objects"]["tiny-yolo-voc"],
                                                   loadedPredictions, trainImgHorsePerson["width"],
                                                   trainImgHorsePerson["height"], 0.01, 0.001)

    assert not (
        differentThanExpectedBike and differentThanExpectedHorse), "The generated object predictions for both images appear to be exactly the same as the ones generated with the original weights.\nTraining may not have completed successfully.\n\nNOTE: It is possible this is a fluke error and training did complete properly (try running this build again to confirm) - but most likely something is wrong."
